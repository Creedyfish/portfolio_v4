...
export default function ExperienceContent() {
  ...
  useEffect(() => {
    listExperiences().then((e) => setExperiences(e as unknown as Experience[]));
  }, []);
          ...
          {visible.map((exp) => {
            ...
            return (
              <div key={exp.id} className="relative">
                {/* Dot */}
                <div
                  className={`absolute top-5 -left-4.25 size-2.5 rounded-full border transition-colors ${
                    isCurrent
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={`shrink-0 font-mono text-lg text-[#22d3ee] transition-transform duration-200 sm:text-xl lg:text-2xl ${
                        isOpen ? "rotate-90" : ""
                      }`}
                    >
                      ›
